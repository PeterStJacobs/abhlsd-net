    ...
    <div class="timeline-item">
      <div class="entry-line entry-line-between">
        ...
        <div class="action-row">
          ...
          <button class="btn-ghost small-btn danger-btn" type="button" data-delete-type="exercise" data-entry-id="${entry.id}">Delete</button>
        </div>
      </div>
    ...
    <div class="timeline-item">
      <div class="entry-line entry-line-between">
        ...
        <div class="action-row">
          ...
          <button class="btn-ghost small-btn danger-btn" type="button" data-delete-type="metric" data-entry-id="${entry.id}">Delete</button>
        </div>
      </div>
    ...
    <div class="timeline-item">
      <div class="entry-line entry-line-between">
        ...
        <div class="action-row">
          ...
          <button class="btn-ghost small-btn danger-btn" type="button" data-delete-type="note" data-entry-id="${entry.id}">Delete</button>
        </div>
      </div>
...
function handleTimelineClick(event){
  ...
  if(deleteButton){
    deleteEntry(deleteButton.dataset.deleteType, deleteButton.dataset.entryId);
    return;
  }
...
function deleteEntry(type, entryId){
  ...
  const confirmed = window.confirm(`Delete this ${labels[type] || 'entry'}? This cannot be undone.`);
  if(!confirmed) return;

  if(type === 'exercise'){
    state.exerciseEntries = state.exerciseEntries.filter((entry) => entry.id !== entryId);
  }else if(type === 'metric'){
    state.metricEntries = state.metricEntries.filter((entry) => entry.id !== entryId);
  }else if(type === 'note'){
    state.noteEntries = state.noteEntries.filter((entry) => entry.id !== entryId);
  }else{
    return;
  }
